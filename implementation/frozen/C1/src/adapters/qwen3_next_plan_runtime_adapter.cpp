...
Args parse_args(int argc, char ** argv) {
    ...
    if (args.arm != "resident_control" && args.arm != "resident_observed_control" &&
        ...
        throw std::invalid_argument(
            ...
            "resident_trace_control, plan_staged_candidate, or plan_cached_candidate");
    }
    if (args.model.empty() || args.backend_dir.empty() ||
        ...
        args.n_ubatch > 4096 || args.prompt_token_limit < 0) {
        throw std::invalid_argument("incomplete or invalid Phase 3 arguments");
    }
    if ((args.arm == "plan_cached_candidate") != (args.cache_capacity > 0) ||
        ...
        try {
            ...
            for (const auto & layer : layers_) {
                ...
                require(layer.down.expert_bytes == 589824 ||
                        layer.down.expert_bytes == 860160,
                        "Qwen3-Next down expert geometry changed");
...
Json run_request(const Args & args) {
    ...
    model_params.load_mode = args.no_mmap ? LLAMA_LOAD_MODE_NONE : LLAMA_LOAD_MODE_MMAP;
    ...
    const std::string formatted = format_prompt(model, prompt_text(args));
    ...
    return {
        ...
        {"workload", {
            ...
            {"prompt_formatter", "official_qwen3_next_one_user_no_tools_thinking_disabled_expansion"},
...
void preflight(const Args & args) {
    ...
    write_json(args.output, {
        ...
        {"geometry", preflight_geometry(args.model)},
        {"layers", kLayers},
        {"experts", kExperts},
        {"experts_used", kExpertsUsed},
