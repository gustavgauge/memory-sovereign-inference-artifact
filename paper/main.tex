...
\title{Memory-Sovereign Inference: Output-Exact Execution Beyond Full Residency}
...
separate from the recorded experiment files.  Audit verifies C1--C12 identities
and assertions offline; replay recomputes reported summaries from packaged
compact evidence.  The principal C1--C3 source and execution contracts are
packaged as reconstruction specifications, not as a claim that a portable
one-command rerun reproduces the recorded certificates.  Model weights and bulk
binary outputs are not redistributed; pinned acquisition metadata, byte lengths,
expected hashes, decisive observations, and validators are supplied.  The stable
interface is
...
Artifact URL: \url{https://github.com/gustavgauge/memory-sovereign-inference-artifact}.
...
\begin{table*}[t]
  ...
  \begin{tabularx}{\textwidth}{@{}p{0.75cm}p{5.2cm}p{5.4cm}X@{}}
    ...
    \midrule
    \hypertarget{claim:C1}{C1} & Semantic demand beyond full residency & Publication claim record & \texttt{8dd54068ed47} \\
    \hypertarget{claim:C2}{C2} & Six-pair complete asynchronous component & Publication claim record & \texttt{29ebe2b94cf3} \\
